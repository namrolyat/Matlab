...
try
    ...
    n_blocks = 12
    n_trials_per_block = 12; % 1 static dots trial, the rest are moving dot trials.
    trial_dur = 12
    
    %Motion directions presented during the localiser
    %0=down, pi=up, pi/2=right,3*pi/2=left
    motion_direction_steps = 180 / (n_blocks*(n_trials_per_block-1) - 1);
    motion_direction = -45:motion_direction_steps:135;
    % convert to radians.
    motion_direction = (motion_direction ./ 360) * 2 * pi;
    percentCoherentMovement = 1;
    
    %Pseudo-randomize trial order
    sequence = randperm(n_blocks*n_trials_per_block);
    ...
    text_results = sprintf('results_localiser_continuous_%d_%d_%d_%d_%d_%d',current_time);
    ...
    for iblock = 1:n_blocks
        ...
        for itrial = 1:n_trials_per_block
            ...
            if sequence((iblock-1)*n_trials_per_block + itrial) > n_blocks*(n_trials_per_block-1)
                %Display static dots
                data{iblock,trial_index} = random_dots(window, width, height, time, trial_dur);
                time = data{iblock,trial_index}.time;
            else
                %Display moving dots
                cur_direction = motion_direction(sequence((iblock-1)*n_trials_per_block + itrial));
                data{iblock,trial_index} = moving_dots(window, width, height, time, trial_dur, cur_direction, percentCoherentMovement,1,0);
                time = data{iblock,trial_index}.time;
            end
            
            if itrial == ceil(n_trials_per_block/2)
