...
FactorStd = 1.5; % This is involved in the typing speed. Bigger std factor = longer typing time, lower std factor = shorter typing time.  
InitiationMarker = 250 ; % This is the marker that will encode the trial number (marker send before and after the trial number marker) 
%%
% ----------------------------------------------------------------------
%                           Experimental input
% ----------------------------------------------------------------------
...
Counter= 1; %Counter for each character that is typed (used in the for loop to make sure timing data is coupled to the correct pressed character
...
try
    ...
    for Trial = 1:length(ConditionRandomizer);
        ...
        if Trial >= 5;
            d=diff(RecordedTime,[],2);                              % Transpose get secs data to milliseconds and the difference to subsequent keypress
        ...
        EEGmarker.sendTrigger(InitiationMarker);
        EEGmarker.sendTrigger(Trial);
        EEGmarker.sendTrigger(InitiationMarker);
        ...
        if ConditionRandomizer(Trial,1) ~= (1);
            ...
            elseif Counter <= ((0.7)*length(Stimulus));
