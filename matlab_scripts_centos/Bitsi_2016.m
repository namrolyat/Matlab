    ...
    methods
        function B = Bitsi_2016(comport)
            ...
            if (not(B.debugmode))
                ...
                fopen(B.serobj);   %previous without parenthesis
        ...
        function setTriggerMode(B)
            fwrite(B.serobj, 0);
            fwrite(B.serobj, 0);
        end
        
        function setLevelMode(B)
            fwrite(B.serobj, 0);
            fwrite(B.serobj, 2);
            fwrite(B.serobj, 255);
        end
        ...
        function [response, timestamp]= getResponse(B, timeout, return_after_response)
            ...
            if (B.debugmode)
                while toc < timeout
                    ...
                    % keyisdown: 1 if any has has been pressed, 0 otherwise
					% when: time key was pressed
					% keyCode: 256-element logical vector indicating which key(s) were pressed
					% deltasecs: interval between this check and the last one
